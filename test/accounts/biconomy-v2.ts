...
async function fixture(): Promise<AccountFixtureReturnType> {
    ...
    return {
        ...
        addSessionKeyCalldata: (key, target, tokens, spendLimit, account) => {
            ...
            // temp: construct using only keys[3], which is the one that is signed with. Use this as the merkle root.
            // Also only uses usdc, since that is what is tested.
